...
public class UserEntityConsumer {
    ...
    public void handleMessage(Message message,Channel channel) throws IOException {
        try {
            // 处理消息
            System.out.println("UserEntityConsumer {} handleMessage :"+message);
            // 执行减库存操作
//            storeService.update(new Gson().fromJson(new String(message.getBody()),UserEntity.class));
            ...
            log.error("UserEntityConsumer  handleMessage {} , error:",message,e);
            // 处理消息失败，将消息重新放回队列
